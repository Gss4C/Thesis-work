...
for index in range(len(crabout[opt.cluster])):
    ...
    for i, infile in enumerate(filenames):
        ...
        for event in range(temp_tree.GetEntries()):
            ...
            h_fwd, l_fwd = conditions_fwdj(tophigh, toplow, jets,l_th, h_th) 
    ...
    plotter(eff_sel_tophigh_fwd, eff_sel_tophigh, eff_sel_toplow_fwd, eff_sel_toplow, opt.threshold, dataset, crabout['meta_info']['eos_plot_path_fwdj'])
saveas_json(output_diz, crabout['meta_info']['eos_count_path_fwdj'] + str(opt.threshold)+"_" +opt.cluster + '.json')